...
import {
  ...
  BRAZIL_STATES,
  type InnovationSpaceSubmissionInput,
} from "../schemas/submission";
import {
  submitInnovationSpaceRequestAction,
  type SubmitState,
...
export function InnovationSpaceRequestForm({
  defaultName,
  defaultEmail,
}: {
  defaultName: string;
  ...
  } = useForm<InnovationSpaceSubmissionInput>({
    resolver: zodResolver(innovationSpaceSubmissionSchema),
    defaultValues: { responsibleName: defaultName, email: defaultEmail },
  });

  const onValid = handleSubmit(async (values) => {
    setServerState(null);
    const result = await submitInnovationSpaceRequestAction(values);
  ...
  return (
    <form onSubmit={onValid} className="mt-6 space-y-4" noValidate>
      ...
      <TextareaField
        ...
        register={register}
        errors={errors}
        hint="Conte em poucas linhas sobre o espaço de inovação."
      />
